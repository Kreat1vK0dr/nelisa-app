  ...
  if (displayValue || (getSalesQuantity && !multiFilters)){
    ...
    dataService.getSalesAllProducts(dateRange, function(err, data){
      ...
      dataService.getPurchasesAllProducts(dateRange, function(err, data){
        ...
        if (compareDates && track===2) {
          dataToSend.purchasesData = data;
          dataToSend.salesData = dataToSend.salesData.map(function(sale){
            var purchaseAtSaleDate = dataToSend.purchasesData.find(function(purchase){ return purchase.date===sale.date;});
            sale.purchases = purchaseAtSale ? +purchaseAtSaleDate.quantity : 0;
            return sale;
          });
        } else if (compareDates && track===1){
          dataToSend.purchasesDataCompare = data;
          dataToSend.salesDataCompare = dataToSend.salesDataCompare.map(function(sale){
            var purchaseAtSaleDate = dataToSend.purchasesDataCompare.find(function(purchase){ return purchase.date===sale.date;});
            sale.purchases = purchaseAtSale ? +purchaseAtSaleDate.quantity : 0;
            return sale;
          });
        } else if (!compareDates && track===1){

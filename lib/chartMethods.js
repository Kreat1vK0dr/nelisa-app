...
var inputDates = dataReceived['inputDates[]'],
    filterBy = dataReceived.filterBy,
    filterRefine = dataReceived.filterRefine,
    display = dataReceived['dataToDisplay[]'],
    dataToDisplay = typeof display === "object" ? display : [display],
    displayType = dataReceived.displayType;

    byProduct = filterBy==="product";
    byCategory = filterBy==="category";
    allProducts = byProduct && filterRefine==="all";
    allCategories = byCategory && filterRefine==="all";
    displayValue = displayType ==="value";
    displayQuantity = displayType ==="quantity";
    compareDates = inputDates.length === 4;
    numberOfFilters = dataToDisplay.length;
    multiFilters = numberOfFilters > 1;

    showTimeLine = ((byProduct || byCategory) && (allProducts || allCategories)) ? false : true;

    getPurchasesQuantity = dataToDisplay.indexOf('purchased')!=-1;
    getSalesQuantity = dataToDisplay.indexOf('sold')!=-1;
  ...
  req.services(function(err,services){
    ...
    dataToSend.dataToShow = dataToDisplay;
...
if (byProduct && allProducts) {
  if (displayValue || (getSalesQuantity && !multiFilters)){
    ...
    dataService.getSalesByProduct([filterRefine,dateRange[0],dateRange[1]], function(err, data){
      if (err) return next(err);
      if (compareDates && track===2) {
        dataToSend.salesData = data;
      } else if (compareDates && track===1){
  ...
  dataService.getPurchasesByProduct([filterRefine,dateRange[0],dateRange[1]], function(err, data){
    if (err) return next(err);
    if (compareDates && track===2) {
      dataToSend.purchasesData = data;
    } else if (compareDates && track===1){
  ...
  if (displayValue || (getSalesQuantity && !multiFilters)){
  dataService.getSalesByCategory([filterRefine,dateRange[0],dateRange[1]], function(err, data){
    if (err) return next(err);
    if (compareDates && track===2) {
      dataToSend.salesData = data;
    } else if (compareDates && track===1){
  ...
  dataService.getPurchasesByCategory([filterRefine,dateRange[0],dateRange[1]], function(err, data){
    if (err) return next(err);
    if (compareDates && track===2) {
      dataToSend.purchasesData = data;
    } else if (compareDates && track===1){

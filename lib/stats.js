...
exports.getWeeks = function(req,res,next){
  var month = req.params.month;

      req.getConnection(function(err, connection){
        connection.query("SELECT week FROM sales WHERE monthname(date)= ? GROUP BY WEEK",[month], function(err, result){
          if (err) return next(err);
          var context = {weeks: result, layout: false};
          res.render('weeks',context);
        });
      });
};

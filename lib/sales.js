...
exports.execute = function (req, res, next) {
    var p_id = Number(req.body.product),
        c_id = Number(req.body.category),
        q = Number(req.body.quantity),
        p = Number(req.body.price),
        date = Date.now();
        context = {};
        console.log(p_id,c_id,q,p,date);
    // req.getConnection(function (err, connection) {
    //     connection.query("SELECT inventory FROM products WHERE id = ?", [p_id], function (err, result) {
    //         if (err) return next(err);
    ...
    //                 var costOfSale = getSaleCalcs.getCostAndLogSaleAt(date, q, purchasesFromDB, purchasesToUpdate);
    ...
    //                             quantity: q,
    //                             price: p,
    //                             cost: costOfSale.cost
    //                           };
    //                             connection.query("INSERT INTO sales SET ?", insertSale, function (err, rows) {
    //                                 if (err) return next(err);
